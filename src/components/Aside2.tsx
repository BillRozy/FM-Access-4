...
const Link = ({
  ...
  return (
    <li className="flex gap-300 basis-[343px] h-min items-center">
      <div className="basis-[100px] grow-0 shrink-0">
        <img className=" aspect-[1/1.27]" src={imgPath} alt={imgAlt}></img>
      </div>
      <section
        className="flex flex-col gap-[11px] basis-[243px] grow justify-between"
        aria-labelledby={sectionId}
      >
        <h2 className="text-red-soft text-[32px] leading-[32px] font-bold">
          0{index + 1}
        </h2>
        <a
          id={sectionId}
          className="font-extrabold text-lg leading-6 hover:text-red-soft"
          href=""
        >
          {topic}
        </a>
...
export default function Aside2() {
  ...
  return (
    <aside
      className="desktop:mt-200"
      aria-label="Section with links to articles"
    >
      <ol className="flex flex-wrap gap-x-[30px] gap-y-400 justify-between">
        {links.map((link, index) => (
          <Link key={link.topic} {...link} index={index}></Link>
        ))}
      </ol>
    </aside>
  );
